...
void remove(const Comparable &x, BinaryNode *&t) {
    ...
    BinaryNode *current_parent = nullptr;
    while (current->element != x && current != nullptr) {
    ...
    if (current == nullptr || current->element != x) return; // 没有找到节点或者节点值不匹配

    // 待删除的节点左右都有孩子
    if (current->left != nullptr && current->right != nullptr) {
        BinaryNode *replacement = detachMin(current->right);
        current->element = replacement->element;
        makeEmpty(replacement);
    } else {
        // 只有一个孩子或没有孩子
