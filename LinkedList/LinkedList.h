...
void SingleLinkedList<T>::insert(T _val)
{
    ...
    else
    {
        ...
        currentPos -> next = newNode ; 
        currentPos = newNode ;//由于没有具体要求，这里将currentPos的位置更新为插入的newNode
    }
    size++;
}
...
void SingleLinkedList<T>::remove()
{
    ...
    if( currentPos -> next == nullptr && currentPos != head ) return ;
    else 
    {
        ...
        if(currentPos != head && currentPos -> next != nullptr )
        {
            Node * tmp = currentPos-> next ;
            currentPos->next = tmp-> next;
            delete tmp;            
        }
    }
}